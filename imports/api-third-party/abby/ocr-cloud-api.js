...
function ProcessingSettings() {
  this.language = "Chinees Simplified"; // Recognition language or comma-separated list
  // of languages.
 ...
 */
ocrsdk.prototype.processImage = function(file, settings, userCallback) {
  // console.log(process.cwd() + file)
  console.log(fs.existsSync(file));
  if (!fs.existsSync(file)) {
    userCallback(new Error("file " + " doesn't exist"), null);
    return;
  }
  ...
  const urlOptions = settings.asUrlParams();
  const req = this._createTaskRequest('POST', '/processImage' + urlOptions,
    userCallback);

  //const fileContents = fs.readFileSync(process.cwd() + file);
